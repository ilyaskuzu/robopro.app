...
export class AvrMcu implements IMicrocontroller {
  ...
  writePin(index: number, value: number): void { const m = this.pinMap[index]; if (!m) return; const pm: Record<string, AVRIOPort> = { B: this.portB, C: this.portC, D: this.portD }; const port = pm[m.portName]; if (port) port.setPin(m.portBit, value > 0); }
